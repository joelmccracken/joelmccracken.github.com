...
a /tree-recursive/ way and the second should compute in a
/tail-recursive/ way.

The point of writing the function in these different ways is to
highlight the fact that tail-recursive formulations are fast and take
...
computation. If the tree-recursive continuation size
continually grows, and the tail-recursive continuation size stays
...
course of the execution, the tree-recursive continuation size kept
growing. Surprisingly, the tail-recursive version kept *precisely the
